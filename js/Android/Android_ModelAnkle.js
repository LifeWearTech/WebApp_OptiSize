...
export function computeWidthFromHeight(heightIn) {
  ...
  const probs = [
    probabilityInRange(3.5, 4.0, est, 0.05) / 100.0,
    probabilityInRange(4.0, 4.5,  est, 0.05) / 100.0,
    probabilityInRange(4.5, 5.0, est, 0.05) / 100.0,
    probabilityInRange(5.0, 5.5,  est, 0.05) / 100.0
  ].map(p => Math.min(Math.max(p, 0), 1));

  const centers = [3.75, 4.25, 4.75, 5.25];
